...
from lung_segmentation import segmentation,PretrainedUNet
...
def test_multi_task(args, random_seed):
    ...
    test_loader = datasets.get_dataset(args.dataset, args.batch_size, configs,
                                       ...
                                       whatsapp_data = True, lung_seg_imagename=True)        
    ...
    for batch_val in tqdm(test_loader):
        ...
        np_lung_seg_filename = lung_segment[0].split(".")
        lung_seg = np.load(np_lung_seg_filename[0]+".npy")
        lung_seg = torch.from_numpy(lung_seg).to(DEVICE)

        if torch.sum(lung_seg) > 0:
    ...
    np.savetxt(f"{results_folder}{args.net_basename}_{args.model_type}_pos_avg2.csv", pos_avg, delimiter=",")
    np.savetxt(f"{results_folder}{args.net_basename}_{args.model_type}_neg_avg2.csv", neg_avg, delimiter=",")
    np.savetxt(f"{results_folder}{args.net_basename}_{args.model_type}_pos_cnt2.csv", pos_cnt, delimiter=",")
    np.savetxt(f"{results_folder}{args.net_basename}_{args.model_type}_neg_cnt2.csv", neg_cnt, delimiter=",")
    np.savetxt(f"{results_folder}{args.net_basename}_{args.model_type}_grad_avg2.csv", grad_avg, delimiter=",")
    np.savetxt(f"{results_folder}{args.net_basename}_{args.model_type}_gradsum_avg2.csv", gradsum_avg, delimiter=",")
